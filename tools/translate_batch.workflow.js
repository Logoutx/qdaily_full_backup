export const meta = {
  ...
  phases: [
    { title: 'Draft', detail: 'Sonnet faithful translation', model: 'sonnet' },
...
log(`Translating ${ids.length} QDaily articles (Sonnet draft -> Opus polish), root=${root}`);

const results = await pipeline(
  ids,
  (id) => agent(draftPrompt(id), { label: `draft:${id}`, phase: 'Draft', model: 'sonnet', schema: DRAFT_SCHEMA }),
  (draft, id) =>
